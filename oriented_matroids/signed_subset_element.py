...
ndset::
    ...
    def __init__(self, parent=None, data = None, groundset = None, positives=None, negatives=None, zeroes = None):
        ...
        if positives is not None:
            if negatives is None:
                raise ValueError("If positives is set, negatives must be as well")
            ...
            if zeroes is None:
                if groundset is None:
                    ...
                    self._z = set(groundset).difference(self._p).difference(self._n)
            else:
                self._z = set(zeroes)
        ...
        # If we have a tuple, use its information
            ...
            if data[0] in [-1,0,1, '+','0','-','']:
                if groundset is not None and len(data) != len(groundset):
                    raise ValueError("Length of vector must be same number of elements as ground set")
        ...
        # If we have a dictionary, use the keys to figure it out
            ...
            if data.has_key('p'):
                self._p = data['p']
            if data.has_key('positives'):
                self._p = data['positives']
            if data.has_key('n'):
                self._n = data['n']
            if data.has_key('negatives'):
                self._n = data['negatives']
            if data.has_key('z'):
                self._z = data['z']
            if data.has_key('zeroes'):
                self._z = data['zeroes']
        else:
            raise ValueError("Either positives and negatives are set or data is a tuple, OrientedMatroidELement or a dict")
    ...
    def __eq__(self,other):
        ...
        if isinstance(other,SignedSubsetElement):
            if self._p == other._p and self._n == other._n and self._z == other._z:
                return True
        return False
    ...
    def reorientation(self,change_set):
        ...
        `{}_{-A}X^+ = (X^+ \backslash A) \cup (X^- \cap A)` and similarly for `{}_{-A}X^-`.
        """
        ...
        p = self.positives().difference(change_set).union(self.negatives().intersection(change_set))
        n = self.negatives().difference(change_set).union(self.positives().intersection(change_set))
